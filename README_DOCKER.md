...
2. Enter: `yacinemassena/mamba-vix-training:latest` (or your Docker Hub username)
...
```bash
# Build
docker build -t yacinemassena/mamba-vix-training:latest .

# Test locally (requires NVIDIA GPU)
docker run --gpus all -it --rm yacinemassena/mamba-vix-training:latest

# Push to Docker Hub
docker login
docker push yacinemassena/mamba-vix-training:latest
```
